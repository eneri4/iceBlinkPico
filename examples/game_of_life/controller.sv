module controller (
    ...
    // state transmitting pixels

    localparam [8:0] TRANSMIT_CYCLES    = 9'd360;
    // = 24 bits / pixel x 15 cycles / bit
    localparam [19:0] IDLE_CYCLES       = 20'd351832;
    // = 375000 - 64 x (360 + 2) for 32 frames / second
    ...
    // 5-bit counter for current frame 0-31 (2^5 = 32)
    logic [8:0] transmit_counter = 9'd0;
    // 9-bit counter for cycles to transmit a frame ((360+2)*64)
    logic [19:0] idle_counter = 20'd0;
    // 20-bit counter for cycles in IDLE or b/t frames
    ...
    // logic assigning when idle done

    // flip-flop b/t states and phases
    // FIX: Change all to posedge clk for synchronization
    ...
    always_comb begin
        ...
        if (state == TRANSMIT_FRAME) begin
            case (transmit_phase)
                ...
                TRANSMIT_PIXEL:
                    next_transmit_phase = transmit_pixel_done ?
                        READ_CH_VALS : TRANSMIT_PIXEL;
            endcase
        end
    end

    // increment pixel counter when done
    // FIX: Change to posedge clk
    always_ff @(posedge clk) begin
